...
### Known issue: pre-commit / formatter fails on `package-lock.json`

If a commit is blocked by a known tooling bug (e.g. formatter choking on `package-lock.json` contents), and the change is strictly a lockfile update:

- Prefer to fix the tool/configuration.
- If you need an urgent PR and the team agrees, you can bypass hooks:

```bash
git commit --no-verify
```

Use this sparingly and add context in the PR description.
